const SentPage = {
  async init() {
    ...
    try {
      const emails = await MailService.getSent();
      EmailList.render(list, emails, { emptyTitle: 'No sent emails', emptyText: 'Emails you send will appear here.' });
    } catch (err) {
      list.innerHTML = `<div class="email-list__empty"><h3>Error loading emails</h3><p>${err.message}</p></div>`;
      Toast.error('Failed to load sent emails: ' + err.message);
    }
  },
...
document.addEventListener('DOMContentLoaded', () => SentPage.init());
